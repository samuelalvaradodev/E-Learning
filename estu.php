...
while ($row = mysqli_fetch_array($result)) {
  //die($row["id_unica"]);
  //$sql = "SELECT*FROM agregar WHERE (id_entrada ={$row['id_unica']}";
 ...
 echo"<tr>";
  echo"<td> <div class='d-flex'>
                                       ...
                                       <h2 class='align-self-center mb-0'> <img src='".$row["perfil"]."' class='img-fluid rounded-circle avatar mr-2'>
                                          <i ></i></h2>
                                        <a href='agregar.php?id=".base64_encode($row["id_unica"])."&val=".$_SESSION["token"]."'>";echo $row["usuario"]; echo " </a> </td></div>";
